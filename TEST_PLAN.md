...
- Non-string JSON serialization coverage verifies an otherwise valid project-like input with runtime `toJSON() => undefined` is blocked with `projectSnapshotUnavailable`.
- No-plan coverage verifies both snapshot failure modes return no `ready` plan, no `alreadyApplied` plan, and no `expectedProjectSnapshot`.
- Normal valid project snapshot coverage verifies `expectedProjectSnapshot` remains exactly `JSON.stringify(project)` without sorting, hashing, parsing, storage normalization, field removal, or field addition.
- Snapshot stability coverage verifies the captured snapshot remains immutable after caller mutation.
- Boundary coverage verifies the correction does not change D.3B authority, D.3A indirect authority, D.3C.2A history validation, idempotency, capacity, drift handling, defensive `sourceIds`, `expectedCurrentValue`, or `writeAuthorized` / `readinessEligible` / `outputEligible` false flags.
- TTI-safety coverage verifies the correction does not clear Draft readiness blockers, infer schema/security/testing/ALM readiness, derive internal names, generate Power Fx, generate YAML, or add repository/storage/readiness/output/UI behavior.
- `npm.cmd ci`: passed; install output reported known development/tooling advisories (`6` vulnerabilities: `2` moderate, `4` high).
- `npm.cmd run lint`: passed.
- `npx.cmd tsc --noEmit -p tsconfig.app.json`: passed.
...
- `npm.cmd run build`: passed with the existing Vite large-chunk warning.
- `npm.cmd audit --omit=dev --audit-level=high`: passed with `0` vulnerabilities.
- `npm.cmd audit --audit-level=high`: failed on known development/tooling advisories (`25` vulnerabilities: `2` moderate, `23` high); dependency remediation remains outside this correction.
...
- `sourceChanged` coverage verifies a post-D stale proposal with coherent deterministic `markStale` history pairs to exactly one same-key generated source replacement when the stale persisted source is non-current, semantic source identity order is unchanged, proposal content is unchanged, and the generated fingerprint differs.
- `ruleChanged` coverage verifies exact old-to-new project-rule source rollover from `projectRule|{ruleId}|{oldVersion}` to `projectRule|{ruleId}|{newVersion}`, including already historical old rule sources, retained readiness source identity, changed rule version, changed fingerprint, and no invented source UUID.
- `applicabilityChanged` coverage verifies applicability-only stale proposals pair without source replacement when source topology remains exact/current and only applicable project types or domains differ.
- Stale no-replacement and exact-match coverage verifies stale proposals with no generated successor or exact current-generation fingerprints block with zero replacement conclusions.
- Coherent stale-history coverage verifies missing `lastDecisionId`, wrong decision action, wrong decision reason, wrong timestamp, and unsafe proposal-ID mismatch fail closed without repair or fabricated decisions.
- Stale reason coverage verifies only `sourceChanged`, `ruleChanged`, and `applicabilityChanged` are accepted; other schema-valid stale reasons such as `targetChanged` block as unsupported.
- Mismatch coverage verifies sourceChanged plus content changes, sourceChanged source topology changes, invalid rule rollover, and applicabilityChanged plus source changes all block without stale-reason reclassification.
- Current lifecycle coverage verifies non-Stale changed proposals, current changed sources, current existing-only proposals, and current existing-only sources block until earlier stale lifecycle/materialization phases run.
- New-planning coverage verifies unrelated generated `newProposal` records are not fabricated into replacement lineages, while mixed new-plus-replacement sets return only the legitimate stale predecessor replacement.
- Ordering coverage verifies multiple replacements sort by canonical proposal key and source replacement pairs sort by replacement source key then stale source ID.
- Shared-source coverage records that canonical clarification blueprints do not naturally create multiple proposals sharing one generated source key; the test documents that limitation instead of fabricating invalid topology.
- Terminal-history coverage verifies `Rejected` and `Superseded` proposals remain historical, are ignored as replacement predecessors, and are not reopened.
- Reconciliation failure coverage verifies malformed or ambiguous normalized planning fails closed with zero replacement conclusions and preserved underlying issue information.
- Input-order and immutability coverage verifies reversed planning/source/proposal/fingerprint order returns equivalent results, caller inputs remain unchanged, and returned nested arrays are defensive copies.
- Isolation coverage verifies no UUID generation, randomness, time, storage, repository writes, source/proposal mutation, decisions, supersession, conflict/dependency changes, readiness integration, generated output integration, UI, network, telemetry, external AI, Wrangler, deployment, PR, tag, release, or `main` integration.
- TTI safety coverage verifies unresolved TTI schema, security, testing, ALM, structured Canvas targets, YAML, delegation, SharePoint internal names, Power Fx, paste-ready YAML, connector/scope blockers, readiness, and output gates are not resolved by replacement analysis.
...
- Focused stale-propagation regression passed (`1` file, `18` tests): `src/tests/planningClarificationStalePropagation.test.ts`.
- Focused lifecycle-analysis regression passed (`1` file, `23` tests): `src/tests/planningClarificationLifecycleAnalysis.test.ts`.
- Focused reconciliation regression passed (`1` file, `12` tests): `src/tests/planningClarificationReconciliation.test.ts`.
- Focused source-reconciliation regression passed (`1` file, `11` tests): `src/tests/planningClarificationSourceReconciliation.test.ts`.
...
- Existing-only rollover coverage remains in place to verify a current old project-rule source absent from generated sources is reported as `noLongerGenerated` and the proposal receives `ruleChanged` only for the exact old-to-new project-rule version source identity change.
- Exact source-set coverage verifies the rule-version rollover exception requires the existing proposal source identity order to match the generated proposal source order with only the project-rule version identity changed.
- Extra source rejection coverage verifies an unrelated additional project-rule source referenced by the persisted proposal keeps the proposal `ambiguous`, leaves stale reason undefined, and does not fabricate lineage or return `ruleChanged`.
- Missing source rejection coverage verifies a rollover proposal missing the readiness-prerequisite source identity remains `ambiguous` with unresolved proposal ambiguity instead of silently accepting a subset.
- Rule-set-only coverage verifies a valid legacy proposal rule-set version with unchanged exact sources is analyzed as `ruleChanged` through the lifecycle-only comparison path.
- Rule-set plus source-change coverage verifies a valid legacy proposal rule-set version combined with a source-set difference remains `ambiguous` and does not receive `ruleChanged`.
- Multiple-reason coverage verifies rule plus applicability and rule plus content changes return `ambiguous` with `multipleLifecycleReasons` and no stale-reason winner.
- Realistic rollover plus second-cause coverage verifies a project-rule source rollover combined with an applicability change returns `ambiguous` with `multipleLifecycleReasons`.
- Existing applicability plus content coverage remains in place to verify non-rule multi-category behaviour still returns `multipleLifecycleReasons`.
- Regression coverage preserves non-terminal `Confirmed` status exclusion, fingerprint-only unresolved ambiguity without `multipleLifecycleReasons`, exact `unchanged`, source `sourceChanged`, existing-only records, terminal history, reconciliation fail-closed behaviour, input-order invariance, defensive copies, and immutability.
- Isolation coverage verifies no UUID generation, randomness, timestamps, `Date`, repository writes, storage/localStorage, stale mutation, `staleAt`, supersession, planning decisions, conflicts, dependencies, UI, hooks, readiness integration, output integration, Power Fx, YAML, generated documents, manifest, ZIP/export, remote persistence, network, telemetry, or external AI is introduced.
- Review-before-main governance coverage verifies the correction remains on `review/phase-5c2-2b-clarification-lifecycle-analysis`, committed and pushed for Architect re-review only, with `main` unchanged and integration blocked pending explicit approval.
- `npm.cmd ci`: passed; install output reported existing development audit advisories (`6` vulnerabilities: `2` moderate, `4` high).
- `npm.cmd run lint`: passed.
- `npx.cmd tsc --noEmit -p tsconfig.app.json`: passed.
...
- Architect correction 5C.2.1F.1 coverage verifies persisted project-rule locator payloads containing `|`, project-rule versions containing `|`, and readiness-prerequisite locator payloads containing `|` return `unrecognizedExistingSourceIdentity` with the correct field, do not derive malformed source keys, do not enter `existingOnly`, are not selected as exact/changed matches, and do not mutate input.
...
- Historical compatibility coverage verifies structurally valid historical source identities are not required to match an active rule registry entry or current generated target before being reported as `noLongerGenerated`.
...
- Final review confirmed the High source-precedence defect was corrected by explicit source authority and full-source-reference precedence.
- Source authority remains separate from availability; non-current sources return no active precedence.
- Correct precedence order is confirmed: current confirmed `userAnswer`, approved document, confirmed intake, approved platform/project/readiness rules, approved project-type preset, informational `userAnswer`, informational derived dependency, informational general recommendation.
- Precedence remains explanatory and cannot resolve conflicts, mutate records, confirm proposals, update intake, affect readiness, affect output, or upgrade informational authority.
- Planning remains optional on `ProjectRecord`.
- No createProject integration, storage migration, repository persistence, readiness integration, generated-output integration, UI, external AI, identity, authentication, dependency, CI, deployment, package, or Wrangler behavior changed.
- Manual browser verification is not applicable because no UI changed.
- `npm.cmd ci`: passed; install output reported the existing high-severity development audit advisory.
- `npm.cmd run lint`: passed.
- `npx.cmd tsc --noEmit -p tsconfig.app.json`: passed.
...
- Phase scope remains limited to the exact five approved files: `src/lib/planningProposals.ts`, `src/tests/planningProposals.test.ts`, `src/types/project.ts`, `CHANGE_LOG.md`, and `TEST_PLAN.md`.
- Final review identified a High source-precedence defect because `sourceType` alone could not distinguish confirmed and unconfirmed user answers.
- Every planning source reference now requires explicit planning authority: `confirmed`, `approved`, or `informational`.
- Source authority is separate from availability; `current` sources may receive active precedence while `stale`, `missing`, `deleted`, and `unverified` sources return no active precedence.
...
- Precedence remains explanatory and cannot select a conflict winner, confirm a source or proposal, change readiness, change generated output, or upgrade informational authority.
- No identity, authentication, persistence, migration, repository integration, readiness integration, generated-output integration, UI, external AI, dependencies, CI, deployment, or Wrangler behavior changed.
- Final integration review remains pending.
- Manual browser verification remains not applicable because no UI changed.
- Focused tests cover current confirmed user answer precedence `1`, current informational user answer precedence `6`, approved document precedence `2`, confirmed intake precedence `3`, approved platform/project/readiness precedence `4`, approved project-type preset precedence `5`, informational derived dependency precedence `7`, informational general recommendation precedence `8`, and non-current source `null` precedence.
- Focused tests cover invalid source-type/authority combinations, missing authority, unknown authority, structured diagnostics without raw source excerpts, removal of proposals referencing removed authority-invalid sources, unchanged readiness isolation, unchanged output isolation, absence of identity/authentication fields, and TTI-like Draft safety.
- `npm.cmd ci`: passed; install output reported the existing high-severity development audit advisory.
- `npm.cmd run lint`: passed.
...
- Phase scope is limited to the exact five approved files: `src/lib/planningProposals.ts`, `src/tests/planningProposals.test.ts`, `src/types/project.ts`, `CHANGE_LOG.md`, and `TEST_PLAN.md`.
- Planning remains optional on `ProjectRecord` and unused by the application.
- No migration, persistence integration, repository operations, duplication behavior, archive/restore/delete behavior, readiness integration, generated-document integration, Package Preview integration, export integration, UI, external AI, dependencies, CI, deployment, or Wrangler behavior changed.
- Normalization covers empty planning state creation, unsupported schema, non-object input, sparse collections, input immutability, all approved categories, statuses, uncertainty states, high-risk restrictions, target kinds, target operations, proposal-value kinds, source types, source availability values, source precedence, UUIDs, fingerprints, strict timestamps, invalid dates and offsets, and foreign project binding rejection.
- State transition coverage verifies every approved valid transition, every invalid transition, same-state rejection, and terminal `Rejected`/`Superseded` behavior.
- Duplicate and corrupt-data coverage verifies every duplicate ID in sources, proposals, decisions, dependencies, and conflicts is removed; malformed individual records are dropped; invalid cross references are removed; invalid `lastDecisionId` and status mismatches invalidate proposals; dependency cycles remove participating dependencies; unknown dependency targets are removed; stale metadata is required for stale proposals; and conflict severity/blocking consistency is enforced.
- Alternative coverage verifies groups remain deterministic and multiple recommended alternatives have all recommended flags removed without selecting a winner.
- Text and collection coverage verifies approved caps, line-ending normalization, text limits, structured-record depth/key/size limits, formula-like Power Fx rejection, paste-ready YAML/source rejection, script/executable payload rejection, and ordinary architectural discussion acceptance.
- TTI safety coverage verifies confirmed planning metadata still cannot clear readiness/output, invent SharePoint internal names, invent Canvas screens or controls, create final Power Fx, create paste-ready YAML, remove missing markers, mark TTI Ready for Codex, mark TTI Ready for Export, or change TTI from Draft.
- Manual browser verification is not applicable because no UI changed.
- `npm.cmd ci`: passed; install output reported the existing high-severity development audit advisory.
- `npm.cmd run lint`: passed.
- `npx.cmd tsc --noEmit -p tsconfig.app.json`: passed.
