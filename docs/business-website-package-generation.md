...
The final bounded correction fixes predicate subject binding on `a47a3a3cff60a5d577a1e847d620880eaaecb350`. The exact reproducer `Implement the booking form, excluded integrations remain out of scope` failed before source changes because punctuation alone allowed `excluded` to bind backward and mark the form negative.

Direct predicates still bind after whitespace, with an optional unpunctuated `that` or `which`. After a comma or opening parenthesis, a trailing predicate binds backward only when explicit `that` or `which` establishes same-subject continuation. Therefore `booking form, which is excluded` remains negative and `booking form, which is approved` remains positive, while `booking form, excluded integrations remain out of scope` leaves the form's request-context polarity positive. Positive and negative predicates use the same subject-binding structure, and explicit negative predicates retain precedence.

This correction adds 27 regressions, bringing PR #4 to 332 added tests. Validation passed 462 focused website tests and 3,570 tests / 97 files in each complete runner (3,492 unit/integration plus 78 UI). Coverage is 90.10% statements / 83.29% branches / 95.49% functions / 93.49% lines, above unchanged thresholds. Lint, TypeScript/build, production audit (zero vulnerabilities) and diff checks passed. The 19-document contract, Power Platform, Static Website, other project families, contact deferral, Client Review gates, unchanged input and export integrity remain green. The existing build-size warning remains. CI #98 is historical evidence for the prior head; this correction requires its own successful CI and fresh automated review.

The occurrence-level deterministic scanner and structured N/A/Deferred authority are unchanged. This is a narrow subject-binding correction within the final bounded-English grammar. Further issues requiring general noun interpretation, semantic roles, dependency parsing or arbitrary sentence interpretation must return to the Head Architect rather than expand this parser.

The fifth refinement corrects two accepted findings on `3dde579bd6487b29911165dfde9ba55ce77b20c8` without replacing the occurrence scanner. Four required reproducers failed before source changes. In a phrase such as `no login or authentication errors affecting the booking form`, an earlier capability occurrence followed by `or` incorrectly made the negative concern look complete. In `booking form, which is approved`, incidental punctuation prevented the existing trailing positive predicate from matching.

At a supported relationship boundary, the scanner now examines the text after the final known capability occurrence. Non-empty substantive material means the capability word participates in a larger concern phrase, so negative scope ends before the later capability. Empty text or coordination/determiners alone still permits direct capability coordination such as `no login or authentication`. The rule uses no noun-head, concern-word or adjective dictionary.

Positive and negative trailing predicates now accept bounded comma and parenthesis punctuation before an optional `that` or `which`. Direct negative cues and explicit trailing negative predicates retain precedence. The field-scoped deterministic scanner, structured N/A/Deferred authority and recorded intake remain unchanged. This is the final authorized expansion of the bounded-English compatibility grammar; ambiguous legacy prose should move to explicit structured review rather than further open-ended inference.

The fifth refinement adds 56 regressions, bringing PR #4 to 305 added tests. Validation passed 435 focused website tests and 3,543 tests / 97 files in each complete runner (3,465 unit/integration plus 78 UI). Coverage is 90.09% statements / 83.29% branches / 95.48% functions / 93.49% lines, above unchanged thresholds. Lint, TypeScript/build, production audit (zero vulnerabilities) and diff checks passed. The 19-document contract, Power Platform, Static Website, other project families, contact deferral, Client Review gates, unchanged input and export integrity remain green. The existing build-size warning remains. CI #97 is historical evidence for the fourth head; the fifth head needs its own successful CI and fresh automated review.

The fourth refinement preserves the deterministic occurrence scanner and corrects two accepted findings on `29e332ce874aa49985ecc0e5e51832924a3477ee`. Four required reproducers failed before source changes: `no errors affecting the booking form` incorrectly negated the form, and `no contact form, and booking form is approved` failed to select the approved replacement.

Negative scope now ends at supported relationships after an intervening concern with no completed capability noun. The rule checks local phrase content and the existing capability vocabulary, without an errors/concern exception list or adjective dictionary. Relationship boundaries also complete the preceding noun, so `without a form affecting submissions` and `without persistent data in production` retain the negative polarity already assigned to that capability. This remains a bounded English compatibility grammar, not general linguistic parsing.

The positive predicate check recognizes is/are approved, required, requested and needed before recording an occurrence. It overrides inherited negation for an independent coordinated replacement. Direct negative cues, negative coordination and explicit trailing negative predicates retain precedence; `no contact form is approved` remains excluded. The original field-scoped architecture and structured N/A/Deferred authority are unchanged.

The fourth refinement adds 80 regressions, bringing PR #4 to 249 added tests. Validation passed 379 focused tests and 3,487 tests / 97 files in each complete runner (3,409 unit/integration plus 78 UI). Coverage is 90.09% statements / 83.28% branches / 95.48% functions / 93.49% lines, above unchanged thresholds. Lint, TypeScript/build, production audit (zero vulnerabilities) and diff checks passed. The 19-document contract, Power Platform, Static Website, other project families, contact deferral, Client Review gates, unchanged input and export integrity remain green. The existing build-size warning remains. CI #96 is historical evidence for the third head; the fourth head needs its own successful CI and fresh automated review.
...
The first two PR #4 revisions added 29 and 69 cases; all remain intact. The third refinement added 71 cases, including independently tested polarity spans, adjectives outside the old whitelist, positive resets and negative coordination, all seven capability families and selected-only generated output. Its validation passed 299 focused tests and 3,407 tests / 97 files in each complete runner (3,329 unit/integration plus 78 UI). Coverage was 90.09% statements / 83.27% branches / 95.48% functions / 93.48% lines, above unchanged thresholds. Lint, TypeScript/build, production audit (zero vulnerabilities) and diff checks passed. The 19-document contract, Power Platform, Static Website, other project families, contact deferral and export regressions passed. The existing build-size warning remained. CI #96 subsequently passed on that third head, before the two new P1 findings required the fourth refinement.
