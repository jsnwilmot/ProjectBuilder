...
Local negation covers adjacent without/no/not/neither/nor phrases, supported negative commands, coordinated lists of known capability phrases, and explicit negative predicates such as `form that is not approved`. Bounded determiners/modifiers are allowed between the negator and subject. Unrelated nouns end prefix-negation scope; noun-completion guards preserve `no data loss` and `no analytics errors`. At least one non-negated requested occurrence selects the capability, including a requested booking form alongside an excluded contact form. This remains a bounded English compatibility grammar, not general language understanding; unrecognized descriptions retain legacy behavior and structured N/A/Deferred remain authoritative for ambiguity.
...
The initial 29 cases remain intact. The refinement adds 69 cases for the accepted findings, requested field lists, locally negated subjects, negative commands, unrelated negation, all seven fields, selected-only generated checks/services, data-model applicability and dependent requirements, structured precedence and export integrity. Existing project-family, contact-deferral, manual-gate and 19-document contract regressions passed. Focused validation passed all 228 website tests. Both complete runners passed 3,336 tests / 96 files (3,258 unit/integration plus all 78 UI tests). Coverage passed unchanged thresholds at 90.06% statements / 83.24% branches / 95.48% functions / 93.46% lines. Lint, TypeScript/build, production audit (zero vulnerabilities) and diff checks passed. The existing build-size warning remains; dependencies are unchanged.
