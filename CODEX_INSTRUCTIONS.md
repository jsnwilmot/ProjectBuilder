...
- Preserve `isCanonicalProjectConfirmationUuid(...)` as the Confirmation compatibility wrapper around the core UUID predicate. Controlled Apply may consume the core UUID predicate directly, but its `invalidApplyId`, `invalidUuid`, and `duplicateUuid` issue semantics remain subsystem-owned.
- The Canvas golden-reference suite in `src/tests/projectBuilderCoreExtractionGolden.test.ts` freezes the current extraction safety contract. Broad Project Builder Core extraction beyond the approved SHA-256 and canonical UUID primitives is not yet implemented.
