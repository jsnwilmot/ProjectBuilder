...
const humanProjectId = "tti-human-decision-project";
const humanProposalId = "82000000-0000-4000-8000-000000000001";
const humanProjectRuleSourceId = "83000000-0000-4000-8000-000000000001";
const humanReadinessSourceId = "83000000-0000-4000-8000-000000000002";
const humanUserAnswerSourceId = "83000000-0000-4000-8000-000000000003";
...
const humanThirdDecisionId = "85000000-0000-4000-8000-000000000003";
const humanThirdSourceId = "86000000-0000-4000-8000-000000000003";
const humanFourthDecisionId = "85000000-0000-4000-8000-000000000004";
const humanFourthSourceId = "86000000-0000-4000-8000-000000000004";
...
const humanThirdAt = "2026-07-22T15:15:00.000Z";
const humanFourthAt = "2026-07-22T16:15:00.000Z";
...
function humanStructuredRecordListValue(): PlanningProposalValue {
  return {
    kind: "structuredRecordList",
    value: [
      { order: { kind: "enum", value: "first" }, detail: humanTextValue("First row.") },
      {
        order: { kind: "enum", value: "second" },
        detail: {
          kind: "structuredRecord",
          value: { note: humanTextValue("Nested second row.") }
        }
      }
    ]
  };
...
function humanStructuredValue(overrides: { purpose?: string; inputs?: string } = {}): PlanningProposalValue {
  return {
    kind: "structuredRecordList",
    value: [{
      approvedComponentName: humanTextValue("Licence summary"),
      purpose: humanTextValue(overrides.purpose ?? "Show licence allocation totals"),
      inputs: humanTextValue(overrides.inputs ?? "Licence records"),
...
function humanSources(ruleId = "pp.canvas.components.confirmation"): PlanningSourceReference[] {
  const rule = humanRule(ruleId);
  return [
    {
      sourceId: humanProjectRuleSourceId,
...
function humanProposal(
  ...
  return {
    ...
    sourceIds: [humanProjectRuleSourceId, humanReadinessSourceId],
...
function humanUserAnswerSource(overrides: Partial<PlanningSourceReference> = {}): PlanningSourceReference {
  return {
    sourceId: humanUserAnswerSourceId,
    sourceType: "userAnswer",
    locator: buildPlanningUserAnswerLocator(humanProposalId, humanReviseDecisionId)!,
    label: "User answer",
    authority: "informational",
    availability: "current",
    observedAt: humanTimestamp,
    ...overrides
  };
}
...
function humanReviseDecision(
  ...
  return {
    decisionId: humanReviseDecisionId,
    proposalId: humanProposalId,
    projectId,
    action: "revise",
...
function humanPlanning(
  ...
  return {
    ...
    proposals: [humanProposal(projectId)],
    ...overrides
  };
}
  ...
  return humanPlanning(projectId, {
    ...
    proposals: [
      humanProposal(projectId, ruleId, {
        ...
        lastDecisionId: humanReviseDecisionId
      })
    ],
    decisions: [humanReviseDecision(projectId, value)],
    ...overrides
  });
}

function humanProject(planning: ProjectPlanningState, overrides: Partial<ProjectRecord> = {}): ProjectRecord {
  const project = createProject({
    identity: { id: humanProjectId, projectName: "TTI Software Licence Tracker" },
    intake: { appType: "powerAppsCanvas" },
    generatedDocuments: [{ fileName: "README.md", folder: "00_Project_Overview", content: "# Existing TTI package" }],
...
describe("projectRepository", () => {
  ...
  it("creates repository projects at storage version 7 with canonical empty controlled apply history", () => {
    const storage = new MemoryStorage();
  ...
  it("migrates v1, v2, and v3 projects to version 7 with empty planning and history", () => {
  ...
  it("round trips v6 structured record lists and preserves independent valid history", () => {
    const storage = new MemoryStorage();
    const structuredValue = humanStructuredRecordListValue();
    const structuredProject = humanProject(humanRevisedPlanning(humanProjectId, structuredValue));
    const historyProject = createProject({
      identity: { id: "v6-history-round-trip", projectName: "V6 History Round Trip" }
    }, new MemoryStorage());
    const history = [controlledApplyHistoryRecord(historyProject.identity.id)];

    saveStorageState({
      version: 6,
      activeProjectId: structuredProject.identity.id,
      projects: [
        structuredProject,
        {
          ...historyProject,
          planning: controlledApplyPlanning(historyProject.identity.id),
    ...
    const loaded = loadStorageState(storage);
    const loadedStructured = loaded.projects.find((project) => project.identity.id === structuredProject.identity.id)!;
    const loadedHistory = loaded.projects.find((project) => project.identity.id === historyProject.identity.id)!;

    expect(loaded.version).toBe(7);
    expect(loadedStructured.planning?.proposals).toHaveLength(1);
    expect(loadedStructured.planning?.proposals[0]).toMatchObject({
      proposalId: humanProposalId,
      fingerprint: humanFingerprint,
      value: structuredValue
    });
    expect(loadedStructured.planning?.proposals[0].sourceIds).toEqual([
      humanProjectRuleSourceId,
      humanReadinessSourceId,
      humanUserAnswerSourceId
    ]);
    expect(loadedStructured.planning?.decisions[0]).toMatchObject({
      decisionId: humanReviseDecisionId,
      value: structuredValue
    });
    expect(loadedStructured.planning?.sources.map((record) => record.sourceId)).toEqual([
      humanProjectRuleSourceId,
      humanReadinessSourceId,
      humanUserAnswerSourceId
    ]);
    expect(loadedHistory.controlledApplyHistory).toEqual(history);
  });
  ...
  it("does not copy structured record list planning answers into duplicated projects", () => {
    const storage = new MemoryStorage();
    const structuredValue = humanStructuredRecordListValue();
    const source = humanProject(humanRevisedPlanning(humanProjectId, structuredValue));
    saveStorageState({ version: 6, activeProjectId: source.identity.id, projects: [source] }, storage);

    const duplicated = duplicateProject(source.identity.id, storage, "2026-08-01T21:00:00.000Z")!;

    expect(duplicated.planning).toEqual(createEmptyProjectPlanningState());
    expect(duplicated.controlledApplyHistory).toEqual([]);
    expect(getProjectById(source.identity.id, storage)?.planning?.proposals[0].value).toEqual(structuredValue);
  });
  ...
  it("guards human-decision materialization project lookup and project-type boundaries without storage writes or runtime calls", async () => {
    const storage = new MemoryStorage();
    createProject({
      identity: { id: "web-project", projectName: "Web Project" },
      intake: { appType: "webApplication" }
    ...
    await expect(materializeProjectPlanningClarificationHumanDecision("missing-project", {
      proposalId: humanProposalId,
      action: "revise",
      value: humanTextValue()
    }, storage, runtime)).resolves.toMatchObject({ outcome: "projectNotFound" });
    await expect(materializeProjectPlanningClarificationHumanDecision("invalid\nproject", {
      proposalId: humanProposalId,
      action: "revise",
      value: humanTextValue()
    ...
    await expect(materializeProjectPlanningClarificationHumanDecision("web-project", {
      proposalId: humanProposalId,
      action: "revise",
      value: humanTextValue()
  ...
  it("persists a bound Revise then explicit Confirm atomically without readiness, output, or schema-metadata integration", async () => {
    ...
    const answer = humanStructuredValue();
    ...
    expect(loaded.planning?.proposals[0]).toMatchObject({
      ...
      lastDecisionId: humanDecisionId,
      updatedAt: humanMaterializedAt
    });
    expect(loaded.planning?.sources.at(-1)).toMatchObject({
      sourceId: humanSourceId,
      sourceType: "userAnswer",
      locator: buildPlanningUserAnswerLocator(humanProposalId, humanDecisionId),
      label: "User answer",
      authority: "informational",
      availability: "current",
      observedAt: humanMaterializedAt
    });
  ...
  it("persists confirmation by staling the prior informational answer and binding a new confirmed answer source", async () => {
    ...
    expect(result).toMatchObject({
      ...
      decisionId: humanDecisionId,
      createdSourceId: humanSourceId,
      staleSourceId: humanUserAnswerSourceId,
      issues: []
    });
    expect(storage.writes).toBe(writesBefore + 1);
    const planning = loadStorageState(storage).projects[0].planning!;
    expect(planning.sources.find((source) => source.sourceId === humanUserAnswerSourceId)).toMatchObject({
      authority: "informational",
      availability: "stale",
      locator: buildPlanningUserAnswerLocator(humanProposalId, humanReviseDecisionId),
      observedAt: humanTimestamp
    });
    expect(planning.sources.find((source) => source.sourceId === humanSourceId)).toMatchObject({
      authority: "confirmed",
      availability: "current",
      locator: buildPlanningUserAnswerLocator(humanProposalId, humanDecisionId),
      observedAt: humanMaterializedAt
    });
    expect(planning.proposals[0]).toMatchObject({
      status: "Confirmed",
      sourceIds: [humanProjectRuleSourceId, humanReadinessSourceId, humanSourceId],
      lastDecisionId: humanDecisionId
    });
    expect(planning.decisions[0]).toMatchObject({
      decisionId: humanReviseDecisionId,
      action: "revise",
      sourceIds: [humanProjectRuleSourceId, humanReadinessSourceId, humanUserAnswerSourceId]
    });
    expect(planning.decisions.at(-1)).toMatchObject({
      decisionId: humanDecisionId,
      action: "confirm",
    ...
    expect(planning.decisions.at(-1)?.value).toBeUndefined();
    expect(planning.decisions.at(-1)?.reason).toBeUndefined();
  });

  it("persists reopen, replacement revision, confirmation, and Confirmed correction through the real repository", async () => {
    const storage = new CountingStorage();
    const answerA = humanStructuredValue();
    const answerB = humanStructuredValue({ purpose: "Corrected purpose" });
    const answerC = humanStructuredValue({ inputs: "Corrected inputs" });
    saveStorageState({
      version: 6,
      activeProjectId: humanProjectId,
      projects: [humanProject(humanRevisedPlanning(humanProjectId, answerA))]
    }, storage);
    ...
    })).resolves.toMatchObject({ outcome: "persisted", action: "reopen", decisionId: humanDecisionId });
    ...
    await expect(materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      ...
      now: () => humanConfirmedAt,
      uuid: (() => {
        const ids = [humanConfirmDecisionId, humanSourceId];
        let index = 0;
        return () => ids[index++]!;
      })()
    ...
    await materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      ...
      now: () => humanThirdAt,
      uuid: (() => {
        const ids = [humanThirdDecisionId, humanThirdSourceId];
        let index = 0;
        return () => ids[index++]!;
      })()
    });
    await expect(materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      ...
      now: () => humanFourthAt,
      uuid: (() => {
        const ids = [humanFourthDecisionId, humanFourthSourceId];
        let index = 0;
        return () => ids[index++]!;
      })()
    })).resolves.toMatchObject({
      outcome: "persisted",
      action: "revise",
      staleSourceId: humanThirdSourceId,
      createdSourceId: humanFourthSourceId
    });

    const reloaded = loadStorageState(storage).projects[0].planning!;
    expect(reloaded.proposals[0]).toMatchObject({
      status: "Revised",
      value: answerC,
      sourceIds: [humanProjectRuleSourceId, humanReadinessSourceId, humanFourthSourceId],
      lastDecisionId: humanFourthDecisionId
    });
    expect(reloaded.sources.find((source) => source.sourceId === humanUserAnswerSourceId)?.availability).toBe("stale");
    expect(reloaded.sources.find((source) => source.sourceId === humanSourceId)?.availability).toBe("stale");
    expect(reloaded.sources.find((source) => source.sourceId === humanThirdSourceId)).toMatchObject({
      authority: "confirmed",
      availability: "stale"
    });
    expect(reloaded.sources.find((source) => source.sourceId === humanFourthSourceId)).toMatchObject({
      authority: "informational",
      availability: "current"
    });
    expect(reloaded.decisions.map((decision) => decision.action)).toEqual([
      "revise", "reopen", "revise", "confirm", "revise"
    ]);
    expect(reloaded.decisions.filter((decision) => decision.action === "confirm")).toHaveLength(1);
  });

  it("persists answered and unanswered Deferred resume and explicit Confirm after answered resume", async () => {
    const answeredStorage = new CountingStorage();
    const answer = humanStructuredValue();
    saveStorageState({
      version: 6,
      activeProjectId: humanProjectId,
      projects: [humanProject(humanRevisedPlanning(humanProjectId, answer))]
    }, answeredStorage);
    ...
    }, answeredStorage, { now: () => humanMaterializedAt, uuid: () => humanDecisionId });
    await expect(materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      proposalId: humanProposalId,
      action: "reopen"
    }, answeredStorage, { now: () => humanConfirmedAt, uuid: () => humanConfirmDecisionId })).resolves.toMatchObject({
      outcome: "persisted",
      action: "reopen"
    });
    await expect(materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
    ...
    })).resolves.toMatchObject({ outcome: "persisted", action: "confirm" });
    const answered = loadStorageState(answeredStorage).projects[0].planning!;
    expect(answered.proposals[0]).toMatchObject({ status: "Confirmed", value: answer });
    expect(answered.decisions[1]).toMatchObject({ action: "defer", reason: "Waiting for stakeholder review." });
    expect(answered.decisions.map((decision) => decision.action)).toEqual(["revise", "defer", "reopen", "confirm"]);

    const unansweredStorage = new CountingStorage();
    saveStorageState({
      version: 6,
      activeProjectId: humanProjectId,
      projects: [humanProject(humanPlanning())]
    }, unansweredStorage);
    await materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      proposalId: humanProposalId,
      action: "defer",
      reason: "Waiting for the first answer."
    }, unansweredStorage, { now: () => humanMaterializedAt, uuid: () => humanDecisionId });
    await materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      proposalId: humanProposalId,
      action: "reopen"
    }, unansweredStorage, { now: () => humanConfirmedAt, uuid: () => humanConfirmDecisionId });
    const unanswered = loadStorageState(unansweredStorage).projects[0].planning!;
    expect(unanswered.proposals[0]).toMatchObject({
      status: "Needs Clarification",
      sourceIds: [humanProjectRuleSourceId, humanReadinessSourceId]
    });
    expect(unanswered.sources).toHaveLength(2);
    expect(unanswered.decisions.map((decision) => decision.action)).toEqual(["defer", "reopen"]);
    expect(unanswered.decisions[0].reason).toBe("Waiting for the first answer.");
  });

  it("fails reopen closed when the project changes before the repository write", async () => {
    const storage = new ChangingReadStorage();
    saveStorageState({
      version: 6,
      activeProjectId: humanProjectId,
      projects: [humanProject(humanRevisedPlanning())]
    }, storage);
    const runtimeCalls = { now: 0, uuid: 0 };
  ...
  it.each([
    ["reject", "Rejected", { proposalId: humanProposalId, action: "reject", reason: "Client rejected this proposed planning answer." }],
    ["defer", "Deferred", { proposalId: humanProposalId, action: "defer", reason: "Client needs more time to answer." }],
    ["markNotApplicable", "Not Applicable", { proposalId: humanProposalId, action: "markNotApplicable", reason: "Canvas components are not applicable." }]
  ] as const)("persists %s without creating user-answer sources", async (action, resultingStatus, input) => {
    const storage = new CountingStorage();
    saveStorageState({ version: 4, activeProjectId: humanProjectId, projects: [humanProject(humanPlanning())] }, storage);
    const writesBefore = storage.writes;
    const runtimeCalls = { now: 0, uuid: 0 };
    ...
    expect(result.createdSourceId).toBeUndefined();
    expect(result.staleSourceId).toBeUndefined();
    expect(runtimeCalls).toEqual({ now: 1, uuid: 1 });
    expect(storage.writes).toBe(writesBefore + 1);
    const planning = loadStorageState(storage).projects[0].planning!;
    expect(planning.sources).toHaveLength(2);
    expect(planning.proposals[0]).toMatchObject({
    ...
    if (action === "markNotApplicable") {
      expect(planning.proposals[0].value).toEqual({ kind: "notApplicable", reason: input.reason });
      expect(planning.decisions.at(-1)?.value).toEqual({ kind: "notApplicable", reason: input.reason });
      expect(planning.decisions.at(-1)?.reason).toBeUndefined();
    } else {
      expect(planning.proposals[0].value).toEqual({ kind: "clarification", question: humanRule().question });
      expect(planning.decisions.at(-1)?.reason).toBe(input.reason);
      expect(planning.decisions.at(-1)?.value).toBeUndefined();
    }
  });

  it("blocks human-decision materialization when the project changes between preparation and write", async () => {
    const storage = new ChangingReadStorage();
    saveStorageState({ version: 4, activeProjectId: humanProjectId, projects: [humanProject(humanPlanning())] }, storage);
    const runtimeCalls = { now: 0, uuid: 0 };
  ...
  it("reports human-decision persistence failure without false success", async () => {
    clearPersistenceWarning();
    try {
      const callerState = {
        version: 4 as const,
  ...
  it("blocks a repeated human revision after successful persistence without a second write or runtime allocation", async () => {
    const storage = new CountingStorage();
    saveStorageState({ version: 4, activeProjectId: humanProjectId, projects: [humanProject(humanPlanning())] }, storage);
    ...
    const second = await materializeProjectPlanningClarificationHumanDecision(humanProjectId, {
      proposalId: humanProposalId,
      action: "revise",
      value: humanTextValue("Second answer.")
    }, storage, {
      now: () => {
        runtimeCalls.now += 1;
        return humanMaterializedAt;
    ...
    expect(second).toMatchObject({
      outcome: "blocked",
      issues: [expect.objectContaining({ code: "invalidStatusTransition" })]
    });
    expect(runtimeCalls).toEqual({ now: 0, uuid: 0 });
    expect(storage.writes).toBe(writesAfterFirst);
    expect(loadStorageState(storage).projects[0].planning?.decisions).toHaveLength(1);
  });
  ...
  it("keeps legacy-compatible storage keys and does not rewrite current saved records while loading", () => {
    ...
    expect(JSON.parse(storage.getItem(STORAGE_KEY)!)).toMatchObject({ version: 7 });
  });

  it("recovers safely from invalid localStorage data", () => {
    const storage = new MemoryStorage();
    storage.setItem(STORAGE_KEY, "{not-json");
    expect(loadStorageState(storage)).toEqual({ version: 7, activeProjectId: null, projects: [] });
  });
  ...
  it("does not fallback to older keys when the current storage key is corrupt", () => {
    ...
    expect(loaded).toEqual({ version: 7, activeProjectId: null, projects: [] });
    expect(storage.getItem(PREVIOUS_STORAGE_KEY)).not.toBeNull();
  });
  ...
  it("migrates current-key version 3 projects to version 7 with separate empty planning and no fabricated records", () => {
    const storage = new MemoryStorage();
    ...
    expect(migrateStorageState(rawState).version).toBe(7);
    const loaded = loadStorageState(storage);
  ...
  it("migrates version-2 Canvas projects to storage version 7 with empty formula evidence and planning by default", () => {
    const storage = new MemoryStorage();
    const project = createProject({
  ...
  it("migrates version-1 Canvas projects through storage version 7 with empty formula evidence and planning by default", () => {
    const storage = new MemoryStorage();
    const project = createProject({
  ...
  it("resets storage safely", () => {
    ...
    expect(resetStorage(storage)).toEqual({ version: 7, activeProjectId: null, projects: [] });
    expect(storage.getItem(STORAGE_KEY)).toBeNull();
    expect(storage.getItem(PREVIOUS_STORAGE_KEY)).toBeNull();
  ...
  describe("default browser storage fallback", () => {
    it("falls back to a safe empty state when window.localStorage is inaccessible", () => {
      ...
      try {
        expect(loadStorageState()).toEqual({ version: 7, activeProjectId: null, projects: [] });
        expect(() => saveStorageState({ version: 3, activeProjectId: null, projects: [] })).not.toThrow();
        expect(resetStorage()).toEqual({ version: 7, activeProjectId: null, projects: [] });
      } finally {
