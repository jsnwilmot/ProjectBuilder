...
interface PlanningReadinessMappingDefinitionBase {
  ...
  canonicalDestinationPaths: readonly string[];
  canonicalMergePaths: readonly string[];
  canonicalValidatorDependencyPaths: readonly string[];
  validatorId: string;
...
function validatePathCollection(
  ...
  for (const value of input) {
    if (typeof value !== "string" || !CANONICAL_PATH_PATTERN.test(value)) {
